...
public static int 출금(int 잔액, int 출금금액) {  // 함수명 "출금"을 공용저장소에 저장 /  (잔액,출금금액) 이라는 변수를 사용할거임 이 자리에 무조건 입력?
잔액 = 잔액 - 출금금액;

// 위에서 계산한 내용을 돌려준다.
return 잔액;									// 돌려준다. 라는건 ok / 있고 없고의 차이?
}
...
// 목적지 함수 
public static int 목적지선택(String 목적지) { //공용저장소 "목적지선택" 함수는 / 목적지 라는 문자 저장소를 사용하고
if(목적지.equals("수원")) { 				 //이걸 진행해.
return 10000;
